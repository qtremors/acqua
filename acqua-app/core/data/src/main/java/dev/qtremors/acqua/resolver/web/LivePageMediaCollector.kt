...
enum class LivePageExtractionFailure {
    SESSION_EXPIRED,
    CONTENT_UNAVAILABLE,
    NO_MEDIA
}

sealed interface LivePageExtractionOutcome {
...
class LivePageMediaCollector(
    ...
    private val capturedNetworkVideos = linkedMapOf<String, String>()
    ...
    fun captureNetworkVideo(url: String) {
        ...
        val identity = WebLink.mediaAssetIdentity(url)
        capturedNetworkVideos.remove(identity)
        capturedNetworkVideos[identity] = url
        while (capturedNetworkVideos.size > MAX_CAPTURED_VIDEO_URLS) {
            capturedNetworkVideos.remove(capturedNetworkVideos.keys.first())
        }
    }
    ...
    fun consume(payload: JSONObject?, sourceUrl: String): LivePageExtractionOutcome {
        ...
        val hasNext = payload.optBoolean("hasNext")
        sawCarouselAdvance = sawCarouselAdvance || clickedNext || payload.optBoolean("structuredCarousel")
        stablePasses = if (media.size == lastItemCount && !clickedNext && !hasNext) stablePasses + 1 else 0
        lastItemCount = media.size
        val hasExpectedMedia = media.isNotEmpty() && (!expectsVideo || media.values.any(ResolvedMedia::isVideo))

        return when {
            payload.optBoolean("structuredCarousel") && hasExpectedMedia ->
                LivePageExtractionOutcome.Complete(normalizedMedia())
            hasExpectedMedia && !clickedNext && !hasNext && stablePasses >= requiredStablePasses ->
                LivePageExtractionOutcome.Complete(normalizedMedia())
    ...
    private fun collectNetworkVideos(
        ...
        capturedNetworkVideos.values.forEach { url ->
            addMedia(ResolvedMedia(
                url = url,
    ...
    private fun addMedia(candidate: ResolvedMedia) {
        val identity = WebLink.mediaAssetIdentity(candidate.url)
        val existing = media[identity]
        ...
        if (existing == null || candidateArea >= existingArea) media[identity] = candidate
    }
    ...
    private fun normalizedMedia(): List<ResolvedMedia> {
        ...
        val posterUrls = videos.mapNotNull(ResolvedMedia::thumbnailUrl)
            .map(WebLink::mediaAssetIdentity)
            .toSet()
        return items.filterNot {
            !it.isVideo && WebLink.mediaAssetIdentity(it.url) in posterUrls
        }
    }
